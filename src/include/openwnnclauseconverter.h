...
class OpenWnnClauseConverter
{

public:
    OpenWnnClauseConverter();
    ~OpenWnnClauseConverter();

    void setDictionary(OpenWnnDictionary *dict);
    vector<WnnClause> convert(const wstring &input);
    shared_ptr<WnnSentence> consecutiveClauseConvert(const wstring &input);
private:
    unique_ptr<OpenWnnClauseConverterPrivate> p;
};
