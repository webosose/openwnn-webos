...
public:
    ...
    static void convertStringToNjChar(NJ_CHAR *dst, const wstring &srcString, int maxChars)
    {
        string utf8str = wstring_to_utf8(srcString);
    ...
    static wstring convertNjCharToString(const NJ_CHAR *src, int maxChars)
    {
        vector<unsigned char> dst((NJ_MAX_LEN + NJ_MAX_RESULT_LEN + NJ_TERM_LEN) * 3 + 1);
        ...
        dst.resize(o);
        string utfData(dst.begin(), dst.end());
        return utf8_to_wstring(utfData);
    }
    ...
    int searchWord(OpenWnnDictionary::SearchOperation operation, OpenWnnDictionary::SearchOrder order, const wstring &keyString)
    {
    ...
    list<wstring> getApproxPattern(const wstring &src)
    {
        if (src.empty() || src.length() > 1)
            return list<wstring>();
        ...
        for (int i = 0; i < work.approxSet.charset_count; i++) {
            if (nj_strcmp(from, work.approxSet.from[i]) == 0) {
                result.push_back(convertNjCharToString(work.approxSet.to[i], NJ_MAX_CHARSET_TO_LEN));
            }
        }
    ...
    int setStroke(const wstring &stroke)
    {
        if (stroke.empty()) {
            /* If a invalid parameter was specified, return an error code */
    ...
    int setCandidate(const wstring &candidate)
    {
        if (candidate.empty()) {
            /* If a invalid parameter was specified, return an error code */
    ...
    vector<bool> getConnectArray(int leftPartOfSpeech)
    {
        NJ_UINT16 lcount = 0, rcount = 0;
...
{
    p.reset(new OpenWnnDictionaryHelper());
}
...
int OpenWnnDictionary::setDictionary(int index, int base, int high)
{
    ...
    case OpenWnnDictionary::INDEX_LEARN_DICTIONARY:
        ...
        return 0;
    default:
        return p->setDictionaryParameter(index, base, high);
    }
}

void OpenWnnDictionary::clearApproxPattern()
{
    p->clearApproxPatterns();
}

int OpenWnnDictionary::setApproxPattern(const wstring &src, const wstring &dst)
{
    return p->setApproxPattern(src, dst);
}

int OpenWnnDictionary::setApproxPattern(ApproxPattern approxPattern)
{
    return p->setApproxPattern(approxPattern);
}

int OpenWnnDictionary::searchWord(SearchOperation operation, SearchOrder order, const wstring &keyString)
{
    /* Unset the previous word information */
    p->clearResult();
    /* Search to fixed dictionary */
    return p->searchWord(operation, order, keyString);
}

int OpenWnnDictionary::searchWord(SearchOperation operation, SearchOrder order, const wstring &keyString, const WnnWord &wnnWord)
{
    /* Search to fixed dictionary with link information */
    p->clearResult();
    p->setStroke(wnnWord.stroke);
    p->setCandidate(wnnWord.candidate);
    p->setLeftPartOfSpeech(wnnWord.partOfSpeech.left);
    p->setRightPartOfSpeech(wnnWord.partOfSpeech.right);
    p->selectWord();

    return p->searchWord(operation, order, keyString);
}

shared_ptr<WnnWord> OpenWnnDictionary::getNextWord(int length)
{
    /* Get the result from fixed dictionary */
    int res = p->getNextWord(length);
    if (res > 0)
        return make_shared<WnnWord>(p->getCandidate(), p->getStroke(),
                                        WnnPOS(p->getLeftPartOfSpeech(), p->getRightPartOfSpeech()), p->getFrequency());
    return 0;
}

vector<vector<bool>> OpenWnnDictionary::getConnectMatrix()
{
    vector<vector<bool>> result;

    /* 1-origin */
    int lcount = p->getNumberOfLeftPOS();

    for (int i = 0; i < lcount + 1; i++) {
        result.push_back(p->getConnectArray(i));
    }
    return result;
}
...
{
    return WnnPOS(p->getLeftPartOfSpeechSpecifiedType(type), p->getRightPartOfSpeechSpecifiedType(type));
}
